...
#define make_unique(x) sort(all(x)); auto it = unique(all(x)); x.charDPize(it - x.begin())
#define endl "\n"
...
void solveTestCases(){
    ...
    for (int i = 0; i < charDP.size(); i++){
        for (int j = 0; j < charDP[0].size(); j++) {
            if (i == 0 && j == 0){
                charDP[i][j] = '.';
            }
            if (i == 0 && j == 1) {
