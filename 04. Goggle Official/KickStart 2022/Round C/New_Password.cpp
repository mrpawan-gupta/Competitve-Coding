...
void solveTestCases(){
      ...
      bool checkUP = false, checkD = false, checkLow = false, checkS = false;

      F0R(i,n){
        if (isupper(s[i])) checkUP = true;
        if (islower(s[i])) checkLow = true;
        if (isdigit(s[i])) checkD = true;
        if (s[i] == '#' || s[i] == '@' || s[i] == '*' || s[i] == '&') checkS = true;
      }
