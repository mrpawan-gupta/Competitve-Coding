...
void generate(string &s, ll i, string &str, set<string> &st)
{
    if (i == s.length())
    {
        st.insert(str);
...
void solveTestCases()
{
    ...
    for (auto &it : st){
        ll n = it.length();
        bool flag = 1;
        for (ll i = 0; i <= n - 5 && flag; i++){
            if (it[i] == it[i + 4] && it[i + 1] == it[i + 3])
                flag = 0;
        }
        for (ll i = 0; i <= n - 6 && flag; i++) {
            if (it[i] == it[i + 5] && it[i + 1] == it[i + 4] && it[i + 2] == it[i + 3])
                flag = 0;
        }
        if (flag){
            cout << "POSSIBLE" << endl;
