...
void solveTestCases(){
    ...
    vi vals;
    F0R(i, N) {
        if (sz(graph[i]) == 2) vals.pub(i);
    }
    each(a, vals) {
        int X = *graph[a].begin(), Y = *graph[a].rbegin();
        graph[X].erase(a); graph[Y].erase(a);
        graph[X].insert(Y); graph[Y].insert(X);
    }
    ...
    int r = -1;
    F0R(i, N) if (sz(graph[i]) == 1) r = i;
 
    int rr = *graph[r].begin();
    dfs(rr, r, true);
    cout << max(dp[rr], 2) << endl;
}
