        ...
        for (int i = 1; i <= n; i++) 
            for (int j = 1; j <= m; j++)   { 
                ...
                if (x == 1)  { 
                    ...
                    DownDP[i][j] = max(DownDP[i - 1][j], DownDP[i][j - 1]); 
                } 
                else{ 
                    DP[i][j] = max(DP[i - 1][j], DP[i][j - 1]); 
                    DownDP[i][j] = max(DownDP[i - 1][j], DownDP[i][j - 1]) + 1; 
                } 
            } 
 
        int check = count / 2; 
        int max1 = DP[n][m], min1 = count - (DownDP[n][m]); 
        if (min1 <= check && check <= max1) print("YES");
        else print("NO") ;
}
