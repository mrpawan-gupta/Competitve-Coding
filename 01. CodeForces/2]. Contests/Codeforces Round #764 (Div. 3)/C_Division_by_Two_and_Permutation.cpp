...
void solveTestCases(){
    ...
    cin >> a;
    sort(all(a));
    // bool array to store the occurence of Element to satisfy Permutation
    vb isPermutation(n + 1, false);

    loop(i,0,n){
        // cerr<<i<<endl;
        if (a[i] <= n){
            if (isPermutation[a[i]] == false) {
                isPermutation[a[i]] = true;
            }
            else{
                while (a[i] && isPermutation[a[i]] == true){
                    a[i] /= 2;
                }
                isPermutation[a[i]] = true;
            }
            ...
            // else condition
            // the element by 2 till it make number to form permutaion
            while (a[i] > n){
                a[i] /= 2;
            }
            // cout<<i<<" "<<a[i]<<endl;
    ...
    // check if the aarray is permutation
    for (int i = 1; i <= n; i++){
        if (isPermutation[i] == false){
            cout << "NO" << endl;
            return;
        }
    }
