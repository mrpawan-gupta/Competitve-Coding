...
#define make_unique(x) sort(all(x)); auto it = unique(all(x)); x.resultize(it - x.begin())
#define endl "\n"
...
void solveTestCases(){
    ...
    F0R(i,n){
        ...
        ll result = 0;
        for (int j = i - 1; j >= 0; j--){
            result += cnt_min / input[j] + 1;
            cnt_min = input[j] * (cnt_min / input[j] + 1);
        }
        ll cnt_max = 0;
        for (int j = i + 1; j < n; j++){
            result += cnt_max / input[j] + 1;
            cnt_max = input[j] * (cnt_max / input[j] + 1);
        }
        ans = min(ans, result);
    }
    cout << ans << endl;
...
int main(int argc, char const *arginput[]){
    fast;
