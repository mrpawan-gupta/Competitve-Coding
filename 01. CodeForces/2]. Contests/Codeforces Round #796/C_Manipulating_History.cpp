...
template <typename K, typename V, typename Comp = std::less<K>>//find_by_order, order_of_key
...
#define each(i, History) for (auto &i : History)
...
#define init(History,val) memset(History,val,sizeof(History))
...
vector<long long> sieve(int n) {int *History = new int[n + 1](); vector<long long>sieveVect; 
    for (int i = 2; i <= n; ++i)if (History[i] == 0) {sieveVect.push_back(i); 
    for (int j = 2 * i; j <= n; j += i)History[j] = 1;} return sieveVect;}
...
void solveTestCases(){
    ...
    // cout << "Case #" << Tc << ": "; Tc++;
        ...
        for(li i = 0;i < 2*n;i++){
            for(li j = 0;j < History[i].size();j++){
                ordChar[History[i][j] - 'a']++;
            }
        }
        char ans;
        for(li i = 0;i < 26;i++){
