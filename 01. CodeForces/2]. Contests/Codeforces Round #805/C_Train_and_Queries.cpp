...
/*--------- Predefined Function  ---- */
template <typename T>T GCD(T v, T u){ // To Find GCD Of Two Number
    return u ? GCD (u, v % u) : v;
}
template <typename T>T LCM(T v, T u){ // to find LCM of two number 
    return ((T)v * u) / GCD(v, u);
}
template <typename T>T GCD_extended(T v, T u, T &x, T &y){
    x = 1, y = 0;T x1 = 0, y1 = 1, a1 = v, b1 = u;
...
template <typename T>T roundUp(T v, T u) { return v / u + ((v ^ u) > 0 && v % u); } // divide v by u rounded up
template <typename T>T roundDown(T v, T u) { return v / u - ((v ^ u) < 0 && v % u); } // divide v by u rounded down
template <class T> bool ckmin(T &v, const T &u) { return u < v ? v = u, 1 : 0;} // TO check Min
template <class T> bool ckmax(T &v, const T &u) { return v < u ? v = u, 1 : 0;} // TO check Max
template <typename T>T binpow(T v, T u) { v %= mod;T res = 1;while (u > 0) { // Binary Exponentiation
    if (u & 1){res = res * v % mod;} v = v * v % mod;u >>= 1;}return res;}// (calculate A^B)
...
template <typename T>T factorial(T n){if(n==0)return 0;else { //Factorial of v number -----> O(n)
...
auto Dcmp = [](auto v, auto u){ // Descending Lambda comparator 
    return v >= u;
};
auto Acmp = [](auto v, auto u) { // Accending Lambda Comparator 
    return v <= u;
};
...
void ____solveTestCases(){
    ...
    F0R(i, n){
        cin >> ___arr[i];
        __imap[___arr[i]].push_back(i);
    }

    while (q--){
        int v, u;
        cin >> v >> u;
        if(__imap.find(v)==__imap.end() or __imap.find(u)==__imap.end()){
            print("NO");
            continue;
        }
        bool __isok = 1;
        if(__imap[v].front()<= __imap[u].back()){
            print("YES");
            continue;
        }
        print("NO");
