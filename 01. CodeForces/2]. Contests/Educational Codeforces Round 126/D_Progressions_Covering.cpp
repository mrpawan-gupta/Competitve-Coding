...
const int N = 300008;
long long n, k;
long long ans = 0, a[N], f[N], g[N];
long long get(long long ad, long long need){
    if (need % ad == 0)
        return need / ad;
    return need / ad + 1;
}
void update(long long l, long long r, long long sum){
    if (sum < 0)
        return;
    long long num = get(r - l + 1, sum);
    ans += num;
    g[r - 1] += num;
    if (l >= 2)
        g[l - 2] -= num;
...
void solveTestCases(){
    ...
    for (int i = n; i > 0; i--){
        g[i] += g[i + 1];
        f[i] += (f[i + 1] - g[i]);
        update(max(1LL, i - k + 1), i, a[i] - f[i]);
    }
    print(ans);
}
