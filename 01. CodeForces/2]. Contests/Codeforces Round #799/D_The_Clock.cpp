...
#define init(nums,n) memset(nums,n,sizeof(nums))
#ifndef ONLINE_JUDGE
...
void solveTestCases(){
    ...
    cin>>s; 
    cin>>n; 
    set<string> set_clock; 
    ll ans = 0; 
    ll hours = n/60; 
    ll mints = n%60; 
    while(true){ 
      if(set_clock.find(s) != set_clock.end()){ 
        break; 
      } 
        if(ispalindrome(s)){ 
            ans++; 
        } 
        ll hourCurr = stoi(s.substr(0 , 2)); 
        ll minCurr = stoi(s.substr(3 , 2)); 
        hourCurr += hours; 
        minCurr += mints; 
        if(minCurr >= 60){ 
            hourCurr++; 
        ...
        set_clock.insert(s); 
        hourCurr %= 24; 
        string hrS = to_string(hourCurr); 
        string mnS = to_string(minCurr); 
        if(hourCurr < 10){ 
            hrS = "0" + hrS; 
        } 
        if(minCurr < 10){ 
