...
ll lenOfLongSubarr(vll &arr, ll n, ll k)
{
    unordered_map<ll, ll> inMap;
    ll Summm = 0, Maxxxxx = 0;
    for (ll i = 0; i < n; i++)
    {
        Summm += arr[i];
        if (Summm == k)
            Maxxxxx = i + 1;
        if (inMap.find(Summm) == inMap.end())
            inMap[Summm] = i;
        if (inMap.find(Summm - k) != inMap.end())
        {
            if (Maxxxxx < (i - inMap[Summm - k]))
                Maxxxxx = i - inMap[Summm - k];
        }
    }
    return Maxxxxx;
}
...
void solveTestCases(){
    ...
    // cout << "Case #" << Tc << ": "; Tc++;
        ...
        ll count = lenOfLongSubarr(input, len, subsum);
        if (inSum < subsum) cout << "-1\n";
        else cout << len - count << "\n";
}
