...
void solveTestCases(){
	...
	for(int i=1;i<=n;i++)
	{
		if(s[0][i]=='*'&&s[1][i]=='*') a[i]=3;
		else if(s[0][i]=='*'&&s[1][i]=='.') a[i]=1;
		else if(s[0][i]=='.'&&s[1][i]=='*') a[i]=2;
		else a[i]=0;
		if(a[i]) ed=i;
	}
	for(int i=1;i<=n;i++)
	{
		if(lt&&i<=ed) ans++;
		if(a[i]&&!lt) lt=a[i],ans+=(a[i]==3);
		else if(a[i]==3) ans++,lt=3;
		else if(!a[i]) continue;
		else if(lt==3) lt=a[i];
		else if(lt!=a[i]) ans++,lt=3;//cout<<ans<<" "<<lt<<"\n";
	}
	cout<<ans<<"\n";
