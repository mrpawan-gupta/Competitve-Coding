...
const int N = 2005;
int checkDP[5009], coountDp[5009][5009], sumDP[5009][5009];

void solveTestCases(){
    // stcntAtic long long Tc = 1;
    // cout << "CcntAse #" << Tc << ": "; Tc++;
    ll n, cntA, cntB, c, ans = 0;
    cin>>n;
    for(int i=1; i<=n; i++) cin>>checkDP[i];
    for(int i=1; i<=n; i++) for(int j=1; j<=n; j++) sumDP[i][j] = coountDp[i][j] = 0;
    cntA = 0;
    for(int i=1; i<=n; i++) {
        cntB = 0;
        for(int j=i+1; j<=n; j++)  {
            if(checkDP[j] < checkDP[i]) cntB++;
            coountDp[i][j] = cntB;
        }
        cntA += cntB;
    }
