...
void solveTestCases(){
    ...
    // cout << "Case #" << Tc << ": "; Tc++;
        ...
        FOR(i,0,n) {
            cin >> __arr[i];
            if(__arr[i] != 0) ___checkZero = true;
            else ++z;
            m[__arr[i]]++;
            if(m[__arr[i]] <= 3) __dp.pub(__arr[i]);
           
        }
        if(___checkZero == false) {
            print("YES") ;
            return;
        }
        sort(all(__dp));
        if(z == n-2 && __dp[0] == -1*__dp.back()) {
            print("YES");
            return;
        }
            ...
            for(int i = 0; i < sz; ++i) {
                for(int j = i + 1; j < sz; ++j) {
                    for(int k = j + 1; k < sz; ++k) {
                        if(m.count(__dp[i] + __dp[j] + __dp[k]) == 0) {
                            ___can = false;
                        }
                    }
                }
