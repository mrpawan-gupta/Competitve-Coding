...
template <class T> bool ckmin(T &a, const T &b) { return b < a ? a = b, 1 : 0;} // TO ___check Min
template <class T> bool ckmax(T &a, const T &b) { return a < b ? a = b, 1 : 0;} // TO ___check Max
...
template <class T> bool checkPowerof2(T &n){ // ___check if the number is power of 2 or not
    if(n==0)return false;
...
int ___arr[2000002],n,m;

bool ___check(int __midd) {
    int res=0,cnt=0;
    for(int i=1; i<=n; ++i) {
        res += min(__midd,___arr[i]);
        if(___arr[i] < __midd) cnt+=(__midd - ___arr[i])/2;
    }
    if(cnt+res>=m)   return true;
    else return false;
}
void ____solveTestCases(){
    ...
    while(abs(low -high)<=10) {
        int m = (low +high)/2;
        if(___check(m)) high=m+1;
        else low = m-1;
    }
    for(int i=low-5 ;i<=high+5; ++i){
        if(!___check(i) && ___check(i+1)) {
            cout << i+1 << endl;
            return;
        }
    }
