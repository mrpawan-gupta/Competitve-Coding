...
void solveTestCases(){
    ...
    // cout << "Case #" << Tc << ": "; Tc++;
        ...
        vector<pii>makeDP;
        F0R(i,n){
            makeDP.pb({arr[i],move[i]});
        }
        sort(makeDP.begin(),makeDP.end());
        vector<pii>countDP;
        F0R(i,n){
            if(arr[i]==makeDP[i].Fi and  move[i]==makeDP[i].Si){
                continue;
            }
            else {
                for(int j=i+1;j<n;j++){
                    if(arr[j]==makeDP[i].Fi and move[j]==makeDP[i].Si){
                        std::swap(arr[i],arr[j]);
                        std::swap(move[i],move[j]);
                        countDP.pb({i+1,j+1});
                    }
                }
            }
        ...
        bool check = true;
        for(int i=1;i<n;i++){
            if(makeDP[i].Fi >=makeDP[i-1].Fi and makeDP[i].Si>=makeDP[i-1].Si){
              continue;
            }
            else check = false;
        }
