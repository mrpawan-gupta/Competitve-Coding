...
void solveTestCases(){
  ...
  cin >> s;
  int n = s.size();
  vi countOne(n + 2), countZero(n + 2);
  countOne[n + 1] = countZero[0] = 0;

  for (int i = 1; i <= n; i++){
    countZero[i] = countZero[i - 1] + (s[i - 1] == '0');
  }
  for (int i = n; i >= 1; i--){
    countOne[i] = countOne[i + 1] + (s[i - 1] == '1');
  }

  int op = 0;
  for (int i = 1; i <= n; i++){
    if (countZero[i - 1] == 0 and countOne[i + 1] == 0) op++;
  }
