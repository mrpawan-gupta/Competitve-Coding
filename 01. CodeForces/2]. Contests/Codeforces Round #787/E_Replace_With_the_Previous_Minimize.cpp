    ...
    // cout << "Case #" << Tc << ": "; Tc++;
        ...
        for (int i = 1; i < n; i++){
            ...
            if (ch <= str[0])
                ...
                char check = 'a';
                for (int i = 1; i <= 26 && 'a' + i - 1 < ch; i++)
                    if (charMap.find('a' + i - 1) != charMap.end())
                        check = 'a' + i - 1;

                int need1 = ch - check;
                int cnt1 = k;
                int count1 = min(need1, cnt1);
