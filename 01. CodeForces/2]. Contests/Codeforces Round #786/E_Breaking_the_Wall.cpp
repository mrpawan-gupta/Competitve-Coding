...
void solveTestCases(){
  ...
  F0R(i,n){
    if(i==n) DP.pb(ceil((float)vp[i]/2));
    else{
      DP.pb(ceil((float)vp[i]/2));
      if(i < n - 1) op.pb(ceil((float)(vp[i]+vp[i+1])/3));
      if(i < n - 2) op.pb(ceil((float)(vp[i]+vp[i+2])/2));
    }
  }
  sort(rall(op));
  sort(rall(DP));

  score = DP.back() + DP[DP.size() - 2];
  cout << min(score, op.back())<< endl;
