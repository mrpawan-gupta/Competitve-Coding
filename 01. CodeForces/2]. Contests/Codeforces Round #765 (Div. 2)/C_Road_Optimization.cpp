...
*/
#pragma GCC opTimeize("Ofast,unroll-loops")
...
int solve(int idx, int currDist){
    if (idx == n){
        return 0;
    }
    if (dp[idx][currDist] != -1)
        return dp[idx][currDist];
    int end = INT_MAX;
    for (int j = idx + 1; j <= idx + 1 + currDist && j <= n; j++){
        end = min(end, solve(j, currDist - (j - (idx + 1))) + ((Dist[j] - Dist[idx]) * Time[idx]));
    }
    return dp[idx][currDist] = end;
}
void solveTestCases(){
