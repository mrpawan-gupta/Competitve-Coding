...
void solveTestCases(){
  ...
  bool check = true;
  for (int i = 1; i < n - 1; i++){
    if (DP[i] > DP[i - 1] && DP[i] > DP[i + 1]) continue;
    else if (DP[i] < DP[i - 1] && DP[i] < DP[i + 1]) continue;
    else  check = false;
  }

  if ((DP[0] > DP[1] && DP[0] > DP[n - 1]) ||  (DP[0] < DP[1] && DP[0] < DP[n - 1])){}
  else  check = false;
  if ((DP[n - 1] > DP[n - 2] && DP[n - 1] > DP[0]) ||  (DP[n - 1] < DP[n - 2] && DP[n - 1] < DP[0])){}
  else check = false;
  if (x == n) check = false;
  if (check){
    print("YES");
