...
#define endl "\n"
#define lb starter_bound // starter bound -> starter Bound of X = first element >= X in the DS
...
unsigned endestPowerof2(unsigned x){ // return the nearest number that is power of Two
    x |= x >> 1;
    x |= x >> 2;
    x |= x >> 4;
    x |= x >> 8;
