...
long long nCr(long long n, long long r){
    long long netSum=factorial(n)/(factorial (r)*factorial (n-r));
    return netSum; // find nCr combination
}
...
void solveTestCases(){
    ...
    int n;
		...
		for (int i = 0; i < n; ++i) {
			int newpos = prefixZero.size() + prefixOne.size();
			if (s[i] == '0') {
				if (prefixOne.empty()) {
					prefixZero.push_back(newpos);
				} else {
					newpos = prefixOne.back();
					prefixOne.pop_back();
					prefixZero.push_back(newpos);
				}
			} else {
				if (prefixZero.empty()) {
					prefixOne.push_back(newpos);
				} else {
					newpos = prefixZero.back();
					prefixZero.pop_back();
					prefixOne.push_back(newpos);
				}
			}
    ...
    deb(prefixZero);
    deb(prefixOne);
		cout << prefixZero.size() + prefixOne.size() << endl;
		for (auto it : netSum) cout << it + 1 << " ";
		cout << endl;
}
