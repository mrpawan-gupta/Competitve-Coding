...
void solveTestCases(){
    ...
    for (int i = 2; i < n - 1; i += 2){
        ...
        vector<int> now = {possible.Fi, possible.Si, i, i + 1};
        for (int j = 0; j < 4; j++){
            vector<int> x = now;
            x.erase(x.begin() + j);
            ch[j] = {get(x), now[j]};
        ...
        sort(all(ch));
        possible = {ch[0].Si, ch[1].Si};
    }
    if (n % 2 == 1){
        int other = 0;
        while (possible.Fi == other || possible.Si == other){
            other++;
        }
        vector<pair<int, int>> ch(4);
        vector<int> now = {possible.Fi, possible.Si, n - 1, other};
        for (int j = 0; j < 4; j++){
            vector<int> x = now;
            x.erase(x.begin() + j);
            ch[j] = {get(x), now[j]};
    ...
    cout << "! " << possible.Fi + 1 << " " << possible.Si + 1 << endl;
}
