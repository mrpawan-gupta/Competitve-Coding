...
constexpr int zerobits(int x){return __builtin_ctz(x);} // # zero bits occurring before first set bit
...
struct cmp {
	bool operator() (const pair<int, int> &a, const pair<int, int> &b) const {
		int lenA = a.Si - a.Fi + 1;
		int lenB = b.Si - b.Fi + 1;
...
void solveTestCases(){
    ...
    int n;
		...
		for (int i = 1; i <= n; ++i) {
			...
			int id = (curr.Fi + curr.Si) / 2;
			a[id] = i;
			if (curr.Fi < id) segs.insert({curr.Fi, id - 1});
			if (id < curr.Si) segs.insert({id + 1, curr.Si});
		}
    cout << a << endl;
}
