...
void solveTestCases(){
    ...
    if (f.size() == 0) {
        ...
        cout << f[0].first + 1 << " " << f[0].second + 1 << endl;
        return;
    }

    auto chk = [&](int k) -> bool {
        int a1 = -1e9, a2 = 1e9, b1 = -1e9, b2 = 1e9;
        for (auto [x, y]: f) {
            a1 = max(x + y - k, a1);
            a2 = min(x + y + k, a2);
            b1 = max(x - y - k, b1);
        ...
        int x1 = (a1 + b1 + 1) / 2, x2 = (a2 + b2) / 2;
        int y1 = (a1 - b2 + 1) / 2, y2 = (a2 - b1) / 2;
        deb(x1,x2);
        deb(y1,y2);
        if (a1 + b1 > n * 2 - 2 or a2 + b2 < 0 or a1 - b2 > m * 2 - 2 or a2 - b1 < 0) {
            return false;
        }
        return true;
    };
    ...
    int a1 = -1e9, a2 = 1e9, b1 = -1e9, b2 = 1e9, k = l;
    for (auto [x, y]: f) {
        a1 = max(x + y - k, a1);
        a2 = min(x + y + k, a2);
        b1 = max(x - y - k, b1);
    ...
    int x = max((a1 + b1 + 1) / 2, 0), y = max({a1 - x, x - b2, -1});
    cout << x + 1 << ' ' << y + 1 << endl;
}
