...
void solveTestCases(){
    ...
    for (int mask = 1; mask < (1 << fact.size()); mask++){
        auto Fi_bit = get_Fi_bit(mask);
        fact_sum[mask].Fi = fact_sum[mask ^ (1 << Fi_bit)].Fi + fact[Fi_bit];
        fact_sum[mask].Si = sb(mask);
    }
    long long res = sb(n);
    for (auto i : fact_sum){
        if (i.Fi <= n){
            res = min(res, i.Si + sb(n - i.Fi));
        }
    }
    cout << res << endl;
}
