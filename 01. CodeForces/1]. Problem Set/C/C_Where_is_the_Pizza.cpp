...
void DFS(int root, vector<int>* edges, bool& found, vector<bool>& visited, vector<int>& used, int& cnt){
    visited[root] = true;
    cnt++;
    if(used[root])
        found = 1;
    for(auto i : edges[root]){
        if(!visited[i]){
            DFS(i, edges, found, visited, used, cnt);
        }
    }
 }
...
void solveTestCases(){
    ...
    // cout << "Case #" << Tc << ": "; Tc++;
        ...
        for(int i = 0; i < n; i++){
            cin >> b[i];
            edges[a[i] - 1].pb(b[i] - 1);
            edges[b[i] - 1].pb(a[i] - 1);
        }
        vector<int> c(n);
        for(int i = 0; i < n; i++){
            cin >> c[i];
