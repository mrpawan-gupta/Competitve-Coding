...
template <class T> bool ckmin(T &a, const T &b) { return b < a ? a = b, 1 : 0;} // TO checkk Min
template <class T> bool ckmax(T &a, const T &b) { return a < b ? a = b, 1 : 0;} // TO checkk Max
...
bool checkkPowerof2(long long n){ // checkk if the number is power of 2 or not
    if(n==0)return false;
...
void solveTestCases(){
    ...
    int n;
		...
		for(int i=0; i < n; ++i) {
			if(a[i] == 1) one = true;
			if(i < n-1 && a[i]+1 == a[i+1]) check = true;
		}
 
		cout << ((one and check) ? "NO" : "YES") << endl;
}
