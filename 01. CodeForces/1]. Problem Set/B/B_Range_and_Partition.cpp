...
void solveTestCases(){
    ...
    cout << ans.Si.Fi << " " << ans.Si.Si << endl;

    int subarrays_found = 0, curr_sum = 0;
    int last_uncovered = 1;

    for (int i = 0; i < n; i++){
        if (a[i] >= ans.Si.Fi and a[i] <= ans.Si.Si){
            curr_sum++;
        }else {
            curr_sum--;
        }

        if (curr_sum > 0 and subarrays_found + 1 < k){
            cout << last_uncovered << ' ' << (i + 1) << endl;
            last_uncovered = i + 2;
            subarrays_found++;
            curr_sum = 0;
        }
