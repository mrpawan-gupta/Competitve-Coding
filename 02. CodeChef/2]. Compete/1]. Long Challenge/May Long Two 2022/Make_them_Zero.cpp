...
long long __gcd(long long a, long long b){ // to find GCD of two number
...
constexpr int setbit(int x) { return __builtin_popcount(x); } // # of check set
constexpr long long setbit(long long x) { return __builtin_popcountll(x);}
constexpr int zerocheck(int x){return __builtin_ctz(x);} // # zero check occuring before first set bit
constexpr long long zercheck (long long x) { return __builtin_ctz(x);}
...
long long countPrimes(ll n) { if(n<2) return 0;
    ...
    return count; // return the number of prime number even_till N
}
