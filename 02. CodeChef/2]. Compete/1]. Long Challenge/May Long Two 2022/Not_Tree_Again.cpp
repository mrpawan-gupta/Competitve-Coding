...
public:
    ...
    BinaryTrieNode *getZero(){
        return zero;
    }
    BinaryTrieNode *getOne(){
        return one;
    }
    void setZero(BinaryTrieNode *node){
        zero = node;
    }
    void setOne(BinaryTrieNode *node){
        one = node;
    }
};
...
public:
    ll minXor;
    BinaryTrieNode *rootNode;

    BinaryTrie() {
        rootNode = new BinaryTrieNode(0);
        minXor = 1e18;
    }
    void insert(ll node) {
        BinaryTrieNode *currNode = rootNode;
        for (int i = 60; i >= 0; i--){
            ll bit = (1 << i) & node;
            if (bit) {
                if (currNode->getOne() == NULL){
                    currNode->setOne(new BinaryTrieNode(node));
                    currNode = currNode->getOne();
                }
                else {
                    currNode = currNode->getOne();
                    currNode->setMax(node);
                    currNode->setMin(node);
                }
            }
            else{
                if (currNode->getZero() == NULL){
                    currNode->setZero(new BinaryTrieNode(node));
    ...
    void mergeTrie(BinaryTrieNode *rootNode1, BinaryTrieNode *rootNode2){
        rootNode1->setMin(min(rootNode1->getMin(), rootNode2->getMin()));
        rootNode1->setMax(max(rootNode1->getMax(), rootNode2->getMax()));
        if (rootNode2->getOne()) {
            if (rootNode1->getOne()){
                mergeTrie(rootNode1->getOne(), rootNode2->getOne());
            }
            else {
                rootNode1->setOne(rootNode2->getOne());
                ll Min = rootNode1->getOne()->getMin() ^ rootNode1->getZero()->getMax();
                minXor = min(minXor, Min);
            }
        }

        if (rootNode2->getZero()){
            if (rootNode1->getZero()) {
                mergeTrie(rootNode1->getZero(), rootNode2->getZero());
            }
            else {
                rootNode1->setZero(rootNode2->getZero());
                ll Min = rootNode1->getOne()->getMin() ^ rootNode1->getZero()->getMax();
                minXor = min(minXor, Min);
            }
        }
...
vvl  Adj;
vll Trie, result;
BinaryTrie *solveXOR(int node, int parent){
    BinaryTrie *trie = new BinaryTrie();
    trie->insert(Trie[node-1]);
    bool check = true;
    for (auto it : Adj[node]) {
        if (it == parent) continue;
        check = false;
        BinaryTrie *tempNode = solveXOR(it, node);
        trie->minXor = min(trie->minXor, tempNode->minXor);
        trie->mergeTrie(trie->rootNode, tempNode->rootNode);
    }
    if (check) result[node - 1] = -1;
    else result[node - 1] = trie->minXor;
    return trie;
}
