...
void get_suffix_prefix(vector<int> &pre, vector<int>&nums,vector<int> &suff, int n){
    pre[0] = nums[0];
    for(int i = 1; i < n; ++i){
...
inline int getGCD(int left, int right, vector<int>&pre, vector<int> &suff, int n){
    if(left == 0){
        return suff[right+1];
    }
    if(right==n-1){
    ...
    return GCD(pre[left-1],suff[right+1]);
}
void solveTestCases(){
    ...
    get_suffix_prefix(pre,nums,suff,n);
    int res = 0;
    for(int i = 0; i < n; ++i){
        if(getGCD(i,i,pre,suff,n)>1){
            res++;
        }
    }
    cout << res << endl;
