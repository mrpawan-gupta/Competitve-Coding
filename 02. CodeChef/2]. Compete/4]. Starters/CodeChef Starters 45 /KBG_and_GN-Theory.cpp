...
void ____solveTestCases(){
    ...
    long n,q,u,v;
            cin >> n >> q;
            for(long ___=1;___<=q;++___) {
                cin >> u >> v;
                vector<pair<long,long>> adj,___graph;
                for(long ___=2; ___*___ <=u; ++___) {
                    long cn__t = 0;
                    while(u%___==0) {
                        u/=___;
                        cn__t++;
                ...
                for(long ___=2;___*___<=v;++___) {
                    ...
                    if(cn__t!=0)
                        ___graph.pub({___,cn__t});
                }
                if(v!=1) ___graph.pub({v,1});
                adj.pub({maxN,0});
                ___graph.pub({maxN,0});
                reverse(adj.begin(),adj.end());
                reverse(___graph.begin(),___graph.end());
                long __ans = 0;
                while((adj.size()>1)||(___graph.size()>1)) {
                    if(adj.back().Fi==___graph.back().Fi) {
                        __ans+=adj.back().Fi*abs(adj.back().Si-___graph.back().Si);
                        adj.pob();
                        ___graph.pob();
                    }
                    else if(adj.back().Fi<___graph.back().Fi) {
                        __ans+=adj.back().Fi*adj.back().Si;
                        adj.pob();
                    } else {
                        __ans+=___graph.back().Fi*___graph.back().Si;
                        ___graph.pob();
                    }
                }
                cout << __ans << endl;
