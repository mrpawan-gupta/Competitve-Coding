...
template <typename check, typename U> std::istream &operator>>(istream &is, pair<check, U> &p){
...
// mcheck9937 rng(chrono::steady_clock::now().time_since_epoch().count());
...
// constexpr int pct(int x) { return __builtin_popcnt(x); } // # of bits set
...
void solveTestCases(){
    ...
    if (cnt == 1){
        ...
        for (i = 0; i < n; i++){
            if (arr[i] > cnt){
                long long int check = arr[i];
                long long int cnt1 = 0;
                while (arr[i] == check && i < n){
                    while (arr[i] == check && i < n) {
                        i++;
                    }
                    check++;
