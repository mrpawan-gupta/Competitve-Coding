...
class c{
    public:
    bool operator () (pair <ll,char>x, pair<ll,char> y){
        return x.Fi < y.Fi;
    }
};
void solveTestCases(){
    ...
    priority_queue <pair<ll,char>,vector<pair<ll,char>>,c> charQueue;
    for(ll i = 0; i < 26; i++){
        if(dpchar[i]<=0)  {
            ll ok=1;
        }
        else  {
            charQueue.push({dpchar[i],char('a'+i)});
        }
    }
    ...
    while(charQueue.size()){
        vector <pair<ll,char>>tempVec;
        while(charQueue.size() && ( charQueue.top().Si == p || charQueue.top().Si == q)){
           tempVec.pub(charQueue.top());
            charQueue.pop();
        }
        if(charQueue.size()!=0) {
            ll re=-1;
        ...
        pair <ll,char> TopChar = charQueue.top();
        charQueue.pop();
        ans.pub(TopChar.Si);
        TopChar.Fi--;
        if(TopChar.Fi) charQueue.push(TopChar);
        q = p;
        p = TopChar.Si;
        for(auto j:tempVec){
            charQueue.push(j);
