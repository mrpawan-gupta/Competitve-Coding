...
#define sizeM(x) ((long long)(x).size())
...
bool Dcmp(int a,int b) { // Descending comparator 
    return a >= b;
}
bool Acmp(int a,int b) { // Accending Comparator
    return a >= b;
}
...
void solveTestCases(){
  ...
  cin >> inpS;
  ll ans = 0;
  map<char, priority_queue<long long>> Map;
  ll count = 0;
  char c = inpS[0];
  F0R(i,n){
    if (c == inpS[i]) count++;
