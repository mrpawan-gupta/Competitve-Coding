...
constexpr long long OddIdx = 1e9 + 7, inf = 1e18;
constexpr int OddIdx_N = 10e5 + 5; constexpr ld Eps = 1e-9;
...
void solveTestCases(){
    ...
    loop(i,1,n){
        if (i % 2){
            OddIdx[i] =OddIdx[i - 1] + v[i], EvenIdx[i] = EvenIdx[i - 1];
        }else{
            OddIdx[i] =OddIdx[i - 1], EvenIdx[i] = EvenIdx[i - 1] + v[i];
        }
    }
    ...
    while(q--){
        ...
        x--;
        y--;
        s1 =OddIdx[y], s2 = EvenIdx[y];
        if (x > 0){
            s1 -=OddIdx[x - 1], s2 -= EvenIdx[x - 1];
        }
        if (abs(s1 - s2) % 26 == 0){
            cout << "Yes" << endl;
        }else{
