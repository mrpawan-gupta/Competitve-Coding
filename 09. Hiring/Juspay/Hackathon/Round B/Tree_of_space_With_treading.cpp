...
bool NaryTree::lock(Node *root, int userId){
	if(root->isResourceInUse)
        while (root->isResourceInUse) 
            ;
...
bool NaryTree::unlock(Node *root, int userId){
    ...
    while (currNode != NULL) {
		...
		currNode->isResourceInUse = false;
        currNode = currNode->parent;
    }
	return true;
}
bool NaryTree::update(Node *root, int userId){
	...
	while(!q.empty()){
		...
		for(auto tnode:temp->children){
			if(tnode->isLocked == true && userId!=tnode->userId){
				return false;
			}
	...
	while(!q.empty()){
		...
		for(auto tnode:temp->children){
			...
			if(tnode->isLocked){
				if(!unlock(tnode,userId)) return false;
			}
			//if(!unlock(tnode,userId)) return false;
			q.push(tnode);
		}
